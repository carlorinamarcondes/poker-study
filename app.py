...
def build_range_chart(raise_hands=None, call_hands=None, equity_dict=None):
    ...
    for i in range(n):
        for j in range(n):
            ...
            ax.add_patch(rect)
            ax.text(j+0.465, n-1-i+0.63, label,
            ...
            ax.text(j+0.465, n-1-i+0.24, eq_str,
    ...
    ax.legend(handles=legend, loc='lower right', fontsize=17,
...
def draw_card_on_ax(ax, cx, cy, card_str, face_down=False, scale=1.0):
    ...
    if face_down:
        ...
        ax.text(cx, cy + 0.18*scale, rank,
                ha='center', va='center',
                fontsize=int(22*scale), color=clr, fontweight='bold')
        ax.text(cx, cy - 0.22*scale, sym,
                ha='center', va='center',
                fontsize=int(20*scale), color=clr)
...
def show_result(parsed, key_suffix="", board=None):
    ...
    with col_left:
        decisao = parsed.get("decisao_ideal") or parsed.get("decisao_correta") or "?"
        nota_raw = parsed.get("nota")
        try:
            nota = int(float(nota_raw)) if nota_raw not in (None, "", "?") else "?"
        except (ValueError, TypeError):
            nota = "?"
        nota_label = f"{nota} / 10" if nota != "?" else "—"
        st.metric("Nota", nota_label)
        st.markdown("**Decisao ideal:**")
        st.warning(decisao)
        st.markdown("**Analise:**")
...
with aba_simulador:
    ...
    if sim["phase"] == "start":
        opp_type_sel = st.selectbox(
            "Tipo de oponente",
            ["Aleatorio", "Agressivo", "Passivo", "Nit"],
            help="Aleatorio: o tipo e sorteado mas so revelado no showdown — treina leitura de oponente!"
        )
        if opp_type_sel == "Aleatorio":
            st.caption("🎲 O tipo do oponente sera sorteado em segredo. Tente descobrir quem e ele durante a mao!")
        if st.button("Gerar nova mao", type="primary"):
            hand = generate_full_hand()
            if opp_type_sel == "Aleatorio":
                actual_type = random.choice(["Agressivo", "Passivo", "Nit"])
                hand["opp_type"] = actual_type
                hand["opp_type_hidden"] = True
            else:
                hand["opp_type"] = opp_type_sel
                hand["opp_type_hidden"] = False
            strength = get_preflop_strength(hand["cartas"])
            opp_a, opp_v = opp_preflop_action(
                hand["opp_type"], "check", strength, hand["bb"]
            )
            sim.update({
        ...
        # --------------------------------------------------
            ...
            # Revela tipo do oponente se era aleatorio
            if hand.get("opp_type_hidden", False):
                tipo_icons = {"Agressivo": "🔥", "Passivo": "🐢", "Nit": "🧊"}
                icone = tipo_icons.get(hand["opp_type"], "🎲")
                st.info(
                    f"{icone} **Revelacao!** O oponente era do tipo **{hand['opp_type']}**. "
                    "Voce conseguiu ler o perfil dele durante a mao?"
                )
