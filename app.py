...
SUIT_COLORS  = {'h': '#e74c3c', 'd': '#e74c3c', 's': '#ecf0f1', 'c': '#2ecc71'}
...
def build_range_chart(raise_hands=None, call_hands=None):
    ...
    for i in range(n):
        for j in range(n):
            ...
            eq = EQUITY_TABLE.get(label, '-')
            ax.text(j+0.465, n-1-i+0.28, f"{eq}%",
...
def build_full_sim_analysis_prompt(hand, actions_log):
    return f"""
...
Avalie cada decisao do Master rua por rua.
...
with aba_simulador:
    ...
    if sim["phase"] == "start":
        ...
        # --------------------------------------------------
            ...
            st.markdown("**Range recomendado:**")
            fig_range = build_range_chart(
                raise_hands=result.get("range_raise",[]),
                call_hands=result.get("range_call",[])
            )
            st.pyplot(fig_range)
            plt.close(fig_range)
